...
nodomain:
	...
	;to debug
	;MessageBox MB_OK|MB_ICONINFORMATION "port=$SSHDPORT  pass=$SSHDPASS  privsep=$PRIVSEP"

	;handle privsep (prior to calling write sshd_config
	StrCpy $R0 $PRIVSEP
	IntCmp $R0 0 noprivsep
	ExpandEnvStrings $0 %COMSPEC% 
...
xp_2k3:
	ExecWait `"$0" /C netsh firewall add portopening name="OpenSSH" protocol=TCP port=$SSHDPORT mode=ENABLE`
	goto fw_done
vista_plus:
	ExecWait `"$0" /C netsh advfirewall firewall add rule name="OpenSSH" dir=in action=allow service=OpenSSHd enable=yes`
	ExecWait `"$0" /C netsh advfirewall firewall add rule name="OpenSSH" dir=in action=allow program="$INSTDIR\usr\sbin\sshd.exe" enable=yes`
	ExecWait `"$0" /C netsh advfirewall firewall add rule name="OpenSSH" dir=in action=allow protocol=TCP localport=$SSHDPORT`
fw_done:
...
ExecWait `"$0" /C cacls "$INSTDIR\var\empty" /E /R "$4"`
...
ExecWait `"$0" /C icacls "$INSTDIR\var\empty" /reset /c /T`	
ExecWait `"$0" /C icacls "$INSTDIR\var\empty" /inheritance:r /c /T`		
UserMgr::GetCurrentUserName
Pop $3
;;MessageBox MB_OK|MB_ICONINFORMATION "username = $3"
ExecWait `"$0" /C icacls "$INSTDIR\var\empty" /inheritance:d /remove:g $3 /grant:r sshd_server:(OI)(CI)F /c /T`	
ExecWait `"$0" /C icacls "$INSTDIR\var\empty" /setowner sshd_server /c /t`

ExecWait `"$0" /C "$\"$INSTDIR\bin\chown.exe"$\" sshd_server /var/empty`
ExecWait `"$0" /C "$\"$INSTDIR\bin\chmod.exe"$\" 600 /var/empty`
